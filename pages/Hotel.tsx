      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <div className={`w-10 h-6 rounded-full transition-colors relative ${form.is_active ? 'bg-black' : 'bg-zinc-300'}`} onClick={() => set('is_active', !form.is_active)}>
        ...
        <div className="p-6 pt-0 flex gap-3">
          ...
          <button
            ...
            className="flex-1 bg-black text-white py-3 rounded-xl font-semibold hover:bg-zinc-800 disabled:opacity-40"
          >Save Room</button>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 max-h-36 overflow-y-auto">
                {services.map(s => {
                  ...
                  return (
                    <label key={s.id} className={`flex items-center gap-2 p-2.5 rounded-xl border cursor-pointer text-sm transition-colors ${checked ? 'bg-black text-white border-black' : 'border-zinc-200 hover:border-zinc-400'}`}>
                      ...
                      <span className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${checked ? 'border-white bg-white' : 'border-zinc-300'}`}>{checked && <Check className="w-3 h-3 text-black" />}</span>
        ...
        <div className="p-6 pt-0 flex gap-3 sticky bottom-0 bg-white border-t border-zinc-100">
          <button onClick={onClose} className="flex-1 border border-zinc-200 text-zinc-700 py-3 rounded-xl font-semibold hover:bg-zinc-50">Cancel</button>
          <button onClick={handleSave} disabled={!form.room_id || !form.pet_name || !form.owner_name || conflict} className="flex-1 bg-black text-white py-3 rounded-xl font-semibold hover:bg-zinc-800 disabled:opacity-40">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['CASH', 'GCASH', 'SPLIT'] as const).map(m => (
                <button key={m} onClick={() => setMethod(m)} className={`py-2.5 rounded-xl text-sm font-bold border transition-colors ${method === m ? 'bg-black text-white border-black' : 'border-zinc-200 hover:border-zinc-400'}`}>{m}</button>
              ))}
            </div>
          </div>
          ...
          <table className="w-full text-xs border-collapse" style={{ minWidth: 900 }}>
            <thead>
              <tr className="bg-zinc-900 text-white">
                <th className="sticky left-0 bg-zinc-900 w-32 px-3 py-3 text-left font-bold">Room</th>
                {dates.map(d => (
            ...
            <div className="flex gap-3 flex-wrap items-center">
              ...
              <div className="flex bg-zinc-100 p-1 rounded-xl">
                {(['TODAY', 'WEEK', 'MONTH', 'ALL'] as const).map(r => (
                  <button key={r} onClick={() => setHistoryRange(r)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${historyRange === r ? 'bg-white shadow-sm text-black' : 'text-zinc-500 hover:text-black'}`}>{r}</button>
                ))}
              </div>
            </div>
          )}
                ...
                return (
                  <div key={booking.id} className="bg-white p-5 rounded-3xl shadow-sm border border-zinc-100 flex flex-col relative group hover:shadow-md transition-shadow">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div className={`text-xs font-bold px-3 py-1 rounded-full border ${booking.check_in === today ? 'bg-zinc-900 text-white border-zinc-900' : 'bg-white text-zinc-900 border-zinc-200'}`}>
                      ...
                      <div className="flex items-center gap-2">
                        {(booking.status === 'RESERVED' || booking.status === 'CHECKED_IN') && (
                          <div className="flex items-center bg-zinc-50 rounded-lg p-0.5 border border-zinc-100">
                            <button onClick={() => { setEditBooking(booking); setShowBookingForm(true); }} className="p-1.5 text-zinc-400 hover:text-black hover:bg-white rounded-md transition-all" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col">
              ...
              <div className="p-4 border-t border-zinc-100">
                <button onClick={() => setEditRoom({} as HotelRoom)} className="w-full flex items-center justify-center gap-2 bg-black text-white py-3 rounded-xl font-semibold hover:bg-zinc-800">
